...
class OllamaAIAgent:
    def __init__(self):
        self.model = "smollm:1.7b"
    
    async def generate_travel_plan(self, request: AIAgentRequest) -> AIAgentResponse:

        weather_data = self._get_weather_data(request.booking_context.location)
        local_events = self._get_local_events(request.booking_context.location)
        activities = self._get_activities(request.booking_context.location, request.preferences.interests)
        restaurants = self._get_restaurants(request.booking_context.location, request.preferences.dietary_restrictions)
        packing_tips = self._get_packing_tips(request.booking_context.location)
    ...
    async def _generate_day_plans(
        ...
        system_prompt = f"""
            ...
            Trip Details:
            - Destination: {booking_context.location}
        ...
        try:
            response = ollama.chat(
                ...
                messages=[
                    ...
                    {"role": "user", "content": "Please create a detailed travel itinerary for this trip."}
                ]
            )
    ...
    def _get_weather_data(self, location: str) -> List[WeatherData]:
        return [
            WeatherData(
                temperature=22.0,
                condition="Sunny",
                humidity=65.0,
                wind_speed=10.0,
                description=f"Beautiful sunny weather in {location}"
            )
        ]
    
    def _get_local_events(self, location: str) -> List[LocalEvent]:
        return [
            LocalEvent(
                title=f"Local Festival in {location}",
                date=date.today() + timedelta(days=1),
                time="10:00 AM",
                location=location,
                description=f"Annual cultural festival featuring local music, food, and art",
                category="cultural"
            )
        ]
    
    def _get_activities(self, location: str, interests: List[str]) -> List[ActivityCard]:
        activities = []
        
        basic_activities = [
            ActivityCard(
                title=f"City Walking Tour",
                address=location,
                price_tier="moderate",
                duration_hours=2.0,
                tags=["culture", "walking"],
                description=f"Explore the highlights of {location} with a guided walking tour",
                wheelchair_accessible=True,
                child_friendly=True
            ),
            ActivityCard(
                title=f"Local Museum",
                address=location,
                price_tier="budget",
                duration_hours=3.0,
                tags=["culture", "indoor"],
                description=f"Discover the history and culture of {location}",
                wheelchair_accessible=True,
                child_friendly=True
            ),
            ActivityCard(
                title=f"Food Market Visit",
                address=location,
                price_tier="budget",
                duration_hours=1.5,
                tags=["food", "local"],
                description=f"Experience local flavors and ingredients",
        ...
        if "outdoor" in interests:
            basic_activities.append(ActivityCard(
                title=f"Nature Park",
                address=location,
                price_tier="budget",
                duration_hours=4.0,
                tags=["outdoor", "nature"],
                description=f"Enjoy nature trails and scenic views",
                wheelchair_accessible=False,
                child_friendly=True
            ))
        ...
        if "food" in interests:
            basic_activities.append(ActivityCard(
                title=f"Cooking Class",
                address=location,
                price_tier="luxury",
                duration_hours=3.0,
                tags=["food", "learning"],
                description=f"Learn to cook local specialties",
                wheelchair_accessible=True,
    ...
    def _get_restaurants(self, location: str, dietary_restrictions: List[str]) -> List[RestaurantRecommendation]:
        restaurants = [
            RestaurantRecommendation(
                ...
                rating=4.5,
                description=f"Cozy bistro serving fresh local ingredients"
            ),
            RestaurantRecommendation(
                ...
                rating=4.2,
                description=f"Authentic local cuisine in a traditional setting"
            ),
            RestaurantRecommendation(
                name=f"Fine Dining",
                address=location,
                ...
                rating=4.8,
                description=f"Upscale dining with innovative cuisine"
            )
        ]
        ...
        if dietary_restrictions:
            ...
            for restaurant in restaurants:
                if any(restriction in restaurant.dietary_accommodations for restriction in dietary_restrictions):
                    filtered_restaurants.append(restaurant)
            return filtered_restaurants
        
        return restaurants
    
    def _get_packing_tips(self, location: str) -> List[str]:
        return [
    ...
    async def _generate_general_response(self, message: str) -> str:
        try:
            system_prompt = """
            You are a helpful AI travel assistant. You help people plan their trips and answer travel-related questions.
            Be friendly, helpful, and provide detailed, structured responses.
            
            IMPORTANT: Pay close attention to the specific locations mentioned by the user. 
            If the user mentions a specific city or destination, always respond about THAT location, not a different one.
            For example, if they ask about Seattle, respond about Seattle, not Tokyo or any other city.
            
            Format your responses with:
            - Use bullet points (*) for lists and recommendations
            - Provide detailed, helpful information
            - Be specific and actionable
            - Always be accurate about the specific location mentioned
            ...
            return "Hello! I'm your AI travel assistant. How can I help you plan your next trip?"

    async def _generate_general_response_with_history(self, message: str, conversation_history: list) -> str:
        try:
            system_prompt = """
            You are a helpful AI travel assistant. You help people plan their trips and answer travel-related questions.
            Be friendly, helpful, and provide detailed, structured responses.
            ...
            print(f"DEBUG: Sending to Ollama - User message: '{message}'")
            print(f"DEBUG: Full conversation context: {[msg['content'] for msg in messages]}")
            ...
            print(f"DEBUG: Ollama response: '{response['message']['content']}'")
            
            return response['message']['content']
        except Exception as e:
            print(f"Error generating general AI response with history: {e}")
            return "Hello! I'm your AI travel assistant. How can I help you plan your next trip?"
    ...
    async def _generate_ai_response(self, request, day_plans, restaurants) -> str:
        ...
        if request.user_query:
            if request.booking_context:
                ...
                IMPORTANT: Always respond about {request.booking_context.location} specifically, not any other city.
                ...
                - Be specific and actionable
                - Use clear headings when appropriate
                - Don't mention specific dates unless the user asks about them
                """
            else:
                system_prompt = f"""
                You are a helpful travel assistant. The user is asking: "{request.user_query}"
                ...
                - Be specific and actionable
                - Use clear headings when appropriate
                - Don't assume any specific destination, dates, or travel details unless the user mentions them
                """
            ...
            try:
                ...
                print(f"DEBUG: Sending to Ollama - User query: '{request.user_query}'")
                print(f"DEBUG: Full conversation context: {[msg['content'] for msg in messages]}")
                ...
                return f"I'd be happy to help with your travel planning! Please provide your booking details and preferences for a personalized itinerary."
        else:
